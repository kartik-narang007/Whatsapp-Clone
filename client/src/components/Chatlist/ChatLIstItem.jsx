...
function ChatLIstItem({ data, isContactsPage = false }) {
  ...
  const handleContactClick = () =>{
    ...
    return (
    <div
      className={`flex cursor-pointer items-center hover:bg-background-default-hover`}onClick={handleContactClick}
    >
      ...
      <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
        ...
        </div>
        {
          !isContactsPage && (
            <div>
              <span className={`${!data.totalUnreadMessages > 0 ? "text-secondary": "text-icon-green"}text-sm`}>{calculateTime(data.createdAt)}</span>
            </div>
          )
        }
